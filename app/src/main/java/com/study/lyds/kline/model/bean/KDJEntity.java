...
public class KDJEntity {
     ...
     */
    public KDJEntity(List<MarketKLineData> kLineBeens, int n, int m1, int m2) {
        ...
        if (kLineBeens != null && kLineBeens.size() > 0) {
            ...
            MarketKLineData kLineBean = kLineBeens.get(0);
            float high = (float) kLineBean.m_fH;
            float low = (float) kLineBean.m_fL;

            for (int i = 0; i < kLineBeens.size(); i++) {
                kLineBean = kLineBeens.get(i);
                if (i > 0) {
                    if (n == 0) {
                        high = high > kLineBean.m_fH ? high : (float) kLineBean.m_fH;
                        low = low < kLineBean.m_fL ? low : (float) kLineBean.m_fL;
                    } else {
                        int t = i - n + 1;
                        Float[] wrs = getHighAndLowByK(t, i, (ArrayList<MarketKLineData>) kLineBeens);
                        high = wrs[0];
                        low = wrs[1];
                    }
                }
                if (high != low) {
                    rSV = (float) ((kLineBean.m_fC - low) / (high - low) * 100);
                } else {
                    rSV = 0;
                }
     ...
     * @return
     */
    private Float[] getHighAndLowByK(Integer a, Integer b, ArrayList<MarketKLineData> kLineBeens) {
        if (a < 0)
            a = 0;

        MarketKLineData kLineBean = kLineBeens.get(a);
        float high = (float) kLineBean.m_fH;
        float low = (float) kLineBean.m_fL;
