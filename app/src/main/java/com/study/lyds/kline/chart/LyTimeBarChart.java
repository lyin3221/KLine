...
public class LyTimeBarChart extends BarChart {
    private BarBottomMarkerView markerBottom;
    private KTimeData kTimeData;
    private KLineData kLineData;

    public LyTimeBarChart(Context context) {
        super(context);
    }

    public LyTimeBarChart(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public LyTimeBarChart(Context context, AttributeSet attrs, int defStyle) {
     ...
     * @param minuteHelper
     * @param kLineData
     * @param type 表示K线显示日期还是时间 0时间 1日期
     */
    private int type = 0;
    public void setMarker(BarBottomMarkerView markerBottom, KTimeData kTimeData, KLineData kLineData, int type) {
    ...
    public void initMyBarRenderer() {//调用自己的渲染器
        mRenderer = new LyBarChartRenderer(this, mAnimator, mViewPortHandler);
    }
    ...
    protected void drawMarkers(Canvas canvas) {
        ...
        for (int i = 0; i < mIndicesToHighlight.length; i++) {
            ...
            if(kLineData == null){//区分应该显示什么数据
                date = DataTimeUtil.secToTime(kTimeData.getDatas().get((int) e.getX()).m_nUpdateTime);//分时图显示的数据
            }else{
