...
public class RSIEntity {
     ...
     * @param n          几日
     */
    public RSIEntity(ArrayList<MarketKLineData> kLineBeens, int n) {
     ...
     */
    public RSIEntity(ArrayList<MarketKLineData> kLineBeens, int n, float defult) {
        RSIs = new ArrayList<>();
        float sum = 0.0f;
        float dif = 0.0f;
        float rs = 0.0f;
...
//                if (i > 0) {
                if (n == 0) {
                    ...
                    int k = i - n + 1;
                    Float[] wrs = getAAndB(k, i, (ArrayList<MarketKLineData>) kLineBeens);
                    sum = wrs[0];
                    dif = wrs[1];
                }
//                }
...
//                    rsi = 100 - c;
    ...
    private Float[] getAAndB(Integer a, Integer b, ArrayList<MarketKLineData> kLineBeens) {
        if (a < 0)
            a = 0;
        float sum = 0.0f;
        float dif = 0.0f;
        ...
        for (int i = a; i <= b; i++) {
            if (i > a) {
                closeT = (float) kLineBeens.get(i).m_fC;
                closeY = (float) kLineBeens.get(i - 1).m_fC;
