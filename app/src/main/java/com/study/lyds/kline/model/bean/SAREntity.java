...
public class SAREntity {
    public SAREntity(ArrayList<MarketKLineData> kLineBeens, int n) {
        this(kLineBeens, n, NaN);
    }

    public SAREntity(ArrayList<MarketKLineData> datas, int num, float defult) {
        // MARK: - 《SAM一线天指标》 处理算法
        /**
         处理SAM运算
            ...
            for (int index = 0; index < datas.size();index++) {
                ...
                if (index - max_index == num) {
                    ...
                    for (int j = index - num + 1; j< index;j++) {
                        ...
                        float c = (float) datas.get(j).m_fC;
                        int v = datas.get(j).m_nVolume;
...
//                        datas.get(j).extVal["\(self.key(CHSeriesKey.volume))"] = max_vol;
                    ...
                    //每位移一个数，计算是否最大交易量
                    float c = (float) datas.get(index).m_fC;
                    int v = datas.get(index).m_nVolume;
